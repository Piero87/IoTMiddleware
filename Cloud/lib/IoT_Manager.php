...
class IoT_Manager
{
	...
	*/
    function __construct(){
    	try
		{
			$this->db = new PDO("mysql:dbname=db;host=localhost;charset=utf8", "username", "password",array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES 'utf8'"));
	...
	*/
	...
	public function get_rules() {
		...
		$get_rules = $this->db->prepare("SELECT R.id as id_rule, T1.id as id_topic, T1.name as topic_name,K1.id as id_key, K1.name as key_name,T2.id as id_topic_result, T2.name as topic_name_result,K2.id as id_key_result, K2.name as key_name_result,R.condition_type, R.condition_value, R.hold_timer, R.key_type_result, R.key_value_result FROM topics as T1, topics as T2, table_keys as K1, table_keys as K2, rules as R WHERE T1.id = R.id_topic AND K1.id = R.id_key AND T2.id = R.id_topic_result AND K2.id = R.id_key_result");
	    $get_rules->execute();
    ...
    {   
	    ...
	    $check_exist = $this->db->prepare("SELECT * FROM rules WHERE
	    									...
	    									id_key_result = :id_key_result AND key_type_result = :key_type_result AND key_value_result = :key_value_result");
		...
		if ($check_exist->rowCount() == 0) {
			...
			$insert_rule = $this->db->prepare("INSERT INTO rules (id_topic,id_key,condition_type,condition_value,hold_timer,id_topic_result,id_key_result,key_type_result,key_value_result) 
												VALUES (:id_topic,:id_key,:condition_type,:condition_value,:hold_timer,:id_topic_result,:id_key_result,:key_type_result,:key_value_result)");
